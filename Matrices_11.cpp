...
vector<int> llenar_vector(int i)
{
    int cant;
    int dni;
    int codigo;
    vector<int> vect;
    cout << "dni del empleado: " << i + 1 << endl;
    cin >> dni;
    vect.push_back(dni);
    return vect;
}
void ultimo(data d, vector<vector<int>> matriz)
{
    vector<int> aux;
    vector<empleado> aux2;
    ...
    for (int i = 0; i < matriz.size(); i++)
    {
        ...
        dni = aux[0];
        aux2 = d.empleados;
        for (int j = 0; j < aux2.size() && !encontrado; j++)
        {
            if (dni == aux2[j].dni)
            {
                encontrado = true;
        ...
        if (encontrado)
        {
            cout << "Dni del empleado: " << dni << endl
                 << "Nombre del empleado: " << nombre << endl;
            cout << "codigo de producto: " << matriz[i][matriz.size() - 1] << endl;
        }
        else
        {
            cout << "dni no encotrado" << endl;
        }
        encontrado = false;
    }
}
...
void modificar_producto(data &d)
{
    ...
    for (int i = 0; i < d.productos.size() && seguir; i++)
    {
        ...
        if (aux.codigo = codigo)
        {
            ...
            cout << "precio venta" << aux.precio_venta << endl
                 << endl;
...
void agregar_producto(data &d, vector<vector<int>> &matriz)
{
    ...
    while (error)
    {
        ...
        cin >> cantv;
        aux.cant_vendida = cantv;
        cout << "precio de costo" << endl;
        cin >> prec_c;
        aux.precio_costo = prec_c;
        cout << "precio de venta" << endl;
        cin >> prec_v;
        aux.precio_venta = prec_v;
...
int main()
{
    ...
    while (continuar)
    {
        ...
        case 8:
            if (matriz.size() < 1)
            {
                cout << "primero crear una matriz" << endl;
            }
            else
            {
                ultimo(d, matriz);
